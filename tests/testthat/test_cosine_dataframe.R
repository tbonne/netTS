...
test_that("Number of nodes equals number of columns", {
  ...
  expect_equal(as.numeric(cosine_between_nodes(graph.2,graph.3)[1]), 0.5)
  expect_equal(as.numeric(cosine_between_nodes(graph.1.d,graph.2.d, directed = T)[1]), 1)
  expect_equal(as.numeric(cosine_between_nodes(graph.2,graph.3, directed = T)[1]), 0.5)
})
