...
CV: Thanks ! I made sure that the data file we are calling between the 2 functions have the same name (df.j.with and df.j.perm). The upper and lower quantiles The create.network was causing problems (weight = NULL) so I have "forced" it by adding another line attributing the weights to the edges. 
Now you've seen what it looked like when I ran it a 100 times. I just did it on small part of the data. 

```{r}
#takes a dataframe with to and from columns and performs a permutation
permute.directed.associations <- function(df.j.with){
  ...
  if(0.5 > runif(1)){
    ...
    while(no.loops == FALSE){
      ...
      rows.to.switch <- sample(1:nrow(df.j.with),2,F)
      
      #record old order
      old.order <- df.j.with$to
      new.order <- df.j.with$to
      ...
      if(sum(as.character(df.j.with$from)==as.character(new.order) )==0){
        
        df.j.with$to <-  as.character(new.order)
        df.j.perm<- df.j.with
        no.loops=TRUE
    ...
    while(no.loops == FALSE){
      ...
      #record old order
      old.order <- df.j.with$from
      new.order <- df.j.with$from
      ...
      #check to make sure there are no self loops
      if(sum(as.character(new.order)==as.character(df.j.with$to) )==0){
        
        df.j.with$from <- as.character(new.order)
        df.j.perm<- df.j.with
        no.loops=TRUE
...
weekly.perm <- function(windowdata, nPerm=1000, effort=1){
  ...
  for(i in 1:nPerm){
    ...
    df.j.perm <- permute.directed.associations(df.j.with)
    
    #bind the permuted data with the unpermuted data
    df.all.perm<-rbind(df.j.without,df.j.perm)
    ...
    E(Perm.network)$weight<-Edgelist$n/effort
...
while (windowStart + windowsize<=max) {
  ...
  if(nrow(windowdata)>threshold){
    ...
    plot(Window.Network, edge.width=E(Window.Network)$weight*500)
    ...
    #run the permutation
    Perm.CC <- weekly.perm(windowdata , 100, effort=scan.tot)
...
```{r}
# Plot
ggplot(OutcomeRST, aes(x=Day, y=obs))  + geom_ribbon(aes(ymin=lower.perm, ymax=upper.perm), fill="red") + geom_point(color="blue") + geom_line() + labs(x= "2015 to 2017 with a 180 days windowsize - shift +1", y="Closeness in spatial proximity behaviour on RST")
```
