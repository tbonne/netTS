...
#' @param df output dataframe from the nodeWin function
#' @export
#' @import ggplot2
#' @import reshape2
#' @examples
#'
#' ts.out<-nodeWin(event.data=df[1:200,])
#' plot.nodeTS(ts.out)
#'
plot.nodeTS <- function(df.ts){

  df.melt<-melt(node.ts[,-ncol(node.ts)], id=c("windowStart"))
...
#' ts.out<-nodeWin(event.data=df[1:100,], type='cosine')
#' plot.nodeTS(ts.out)
#'
...
#' @examples
#'
#' ts.out<-nodeWin(event.data=baboonAssociations)
#' plot.nodeTS(ts.out)
#'
nodeWin <- function (event.data,nBoot=100,nPerm=100,windowSize =30,windowShift= 1, type="cc",directedNet=T, threshold=30){
