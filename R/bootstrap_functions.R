...
#' @param SRI Wether to use the simple ratio index (Default=FALSE)
#' @param probs The quantiles of the bootrap samples to return (Default=c(0.025,0.975)).
#' @param fullData If a subset dataset is being used to compare bootstrap samples to the full dataset.
#' @importFrom stats cor.test quantile
#' @importFrom igraph set_graph_attr degree
#' @export
#'
#'
convergence.check.boot <- function(data, windowsize=days(30), windowshift=days(1), directed = FALSE, measureFun=degree, corFun = 1,boot.samples=100, SRI=FALSE, probs=c(0.025,0.975), fullData=NULL){
  ...
  while (windowstart + windowsize<=max(data[,3])) {
    ...
    if(is.null(fullData)){
      ...
      #calculate the desired network measure
      g.full <- create.a.network(df.window.full, directed = directed, SRI=FALSE, effort = 1)
