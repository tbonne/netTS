...
Change in the network over time: relative to the first network
```{r, warning=FALSE,message=FALSE}
library(lubridate)
library(plyr)
...
graph.cosine <- graphTS(event.data = groomEvents, windowSize = 30, windowShift = 10, type= "cosineFIXED")

#extract cosine measures at the node level
node.cosine <- nodeTS(event.data = groomEvents, windowSize = 30, windowShift = 10, type= "cosineFIXED")
...
ggplot(df.node.cosine, aes(x=windowStart, y=value, color=node)) + geom_line() + geom_line(data=graph.cosine, aes(x=windowStart, y=cosineFIXED), size=2, col="black") + labs(y="Cosine similarity", x = "Day") + theme_classic()
```
